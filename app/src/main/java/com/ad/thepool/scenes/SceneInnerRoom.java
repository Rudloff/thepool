...
public class SceneInnerRoom extends Scene {
	/**
	 * 
	 */
	private static final long serialVersionUID = -7736857851392313412L;

	Exit exit;

	TextBox help;

	public SceneInnerRoom(int order, Game game) {
		super(order, game);
	}
	...
	public void init() {
		...
		clearScene();
		Map map = new Map("maps/inner_room.lvl", null);
		addMap(map);
		addMouseCursor();
		addMenu();
		...
		help = addTextBox(1, 1, 23, 5, "Ride through mirrors in the inner room...", GraphicsWrapper.COLOR_WHITE, true, RenderTextComponent.POS_LEFT_UP, false, null, null);
		saveCheckPoint(true);
