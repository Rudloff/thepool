...
public class SceneLadder extends Scene {
	/**
	 * 
	 */
	private static final long serialVersionUID = 7684330355313316372L;

	Exit exit;

	TextBox help;

	public SceneLadder(int order, Game game) {
		super(order, game);
	}
	...
	public void init() {
		...
		clearScene();
		Map map = new Map("maps/ladder.lvl", null);
		addMap(map);
		addMouseCursor();
		addMenu();
		...
		help = addTextBox(1, 1, 24, 5, "A ladder back home...", GraphicsWrapper.COLOR_WHITE, true, RenderTextComponent.POS_LEFT_UP, false, null, null);
		saveCheckPoint(true);
	}
