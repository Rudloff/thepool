...
public class SceneHammer extends Scene {
	/**
	 * 
	 */
	private static final long serialVersionUID = 6480553834426681050L;

	Exit exit;

	TextBox help;

	public SceneHammer(int order, Game game) {
		super(order, game);
	}
	...
	public void init() {
		...
		clearScene();
		Map map = new Map("maps/hammer.lvl", null);
		addMap(map);
		addMouseCursor();
		addMenu();
		...
		help = addTextBox(1, 1, 23, 5, "Feed the lasers first...", GraphicsWrapper.COLOR_WHITE, true, RenderTextComponent.POS_LEFT_UP, false, null, null);
		saveCheckPoint(true);
