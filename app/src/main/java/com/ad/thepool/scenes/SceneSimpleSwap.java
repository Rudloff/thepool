...
public class SceneSimpleSwap extends Scene {
	/**
	 * 
	 */
	private static final long serialVersionUID = -5140612141790089104L;
	...
	public SceneSimpleSwap(int order, Game game) {
		super(order, game);
	}
	...
	public void init() {
		super.init();
		clearScene();
		Map map = new Map("maps/simpleswap.lvl", "maps/simpleswap.idx");
		addMap(map);
		addMouseCursor();
		addMenu();
		...
		FontaineCanonUp fonUp = (FontaineCanonUp) searchGObjectBySceneScriptIndex('1').get(0);
		fonUp.getFontaineComponent().setActive(false);
		...
		switchComp.setKey("fon");
		fonUp.getFontaineComponent().setSwitchKey("fon");
		fonUp.getFontaineComponent().setReverseSwitch(true);

		setPlayer(playerB, playerA);
		swapAllowed = true;
		swapDuration = 0;
		help = addTextBox(1, 1, 23, 5, "Free your clone...", GraphicsWrapper.COLOR_WHITE, true, RenderTextComponent.POS_LEFT_UP, false, null, null);
