...
public class ScenePortalroom extends Scene {
	/**
	 * 
	 */
	private static final long serialVersionUID = -8403844112309311973L;
	Exit exit;
	TextBox help;

	public ScenePortalroom(int order, Game game) {
		super(order, game);
	}
	...
	public void init() {
		...
		clearScene();
		Map map = new Map("maps/portal_room.lvl", null);
		addMap(map);
		addMouseCursor();
		addMenu();
		...
		help = addTextBox(1, 1, 23, 5, "Portal your way...", GraphicsWrapper.COLOR_WHITE, true, RenderTextComponent.POS_LEFT_UP, false, null, null);
		saveCheckPoint(true);
